...
class PixArtTransformer2DModel(ModelMixin, ConfigMixin):
    ...
    def __init__(
        ...
        if self.config.norm_type != "ada_norm_single":
            raise NotImplementedError(
                f"Forward pass is not implemented when `patch_size` is not None and `norm_type` is '{self.config.norm_type}'."
            )

        elif self.config.norm_type == "ada_norm_single" and num_embeds_ada_norm is None:
            raise ValueError(
                f"When using a `patch_size` and this `norm_type` ({self.config.norm_type}), `num_embeds_ada_norm` cannot be None."
            )
        ...
        self.out_channels = (
            ...
            if self.config.out_channels is None
            else self.config.out_channels
        )
        if self.config.use_additional_conditions is None:
            if self.config.sample_size == 128:
        ...
        # @Sakura: we are generally working with square patches, if you want to do resolution-flexible ones, see related papers
        ...
        self.norm_out = nn.LayerNorm(
            normalized_shape=self.inner_dim, elementwise_affine=False, eps=1e-6
        )
    ...
    def forward(
        ...
        # hidden_states: (batch_size, hidden_dim, height * patch_size, width * patch_size) -> (batch_size, hidden_dim, height, width)
        ...
        # 3. Output
        # shift, scale: (batch_size, 2, inner_dim) -> chunked to (batch_size, 1, inner_dim) each
        shift, scale = (
        ...
        # (batch_size, height*width, out_dim)
        ...
        output = hidden_states.reshape(
            shape=(
                ...
                height * self.config.patch_size,  # height
                width * self.config.patch_size,  # width
            )
        )
